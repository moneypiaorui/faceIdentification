...
train_data=eigenvectors(:,1:500)' *train_data;
test_data=eigenvectors(:,1:500)' *test_data;
...
pcaldaMeanAccuracy = mean(pcaAccuracy, 2);
pcaldaStdDev = std(pcaAccuracy, 0, 2);

% 显示结果
comparisonTable = table( pcaldaMeanAccuracy, pcaldaStdDev, 'RowNames', {num2str(numSamples(1)), num2str(numSamples(2)), num2str(numSamples(3)), num2str(numSamples(4))});
disp(comparisonTable);